...
public class MainActivity extends FragmentActivity implements
	...
	MainActivityLayout btns = (MainActivityLayout) this
		.findViewById(R.id.btn_layout);
	btnHome = btns.getLeftTop();
	btnHome.setOnClickListener(mOnClickListener);

	btnLook = btns.getCenter();
	btnLook.setOnClickListener(mOnClickListener);

	btnSetting = btns.getLeftBottom();
	btnSetting.setOnClickListener(mOnClickListener);

	btnVideos = btns.getRightTop();
	btnVideos.setOnClickListener(mOnClickListener);

	btnPictures = btns.getRightBottom();
	btnPictures.setOnClickListener(mOnClickListener);
...
//	this.registerReceiver(connectStateChangedReceiver, filter);
    ...
    public void destroyBroadcastReceiver() {
    	LocalBroadcastManager.getInstance(this).unregisterReceiver(connectStateChangedReceiver);
    }
	...
	public void onServiceDisconnected(ComponentName name) {
	    controlService.getConnectStateManager()
		    .removeConnectChangedListener(MainActivity.this);
	    controlService = null;
	}
    };
...
//	Intent intent = new Intent();
//	intent.setClass(this, IpcSettings.class);
//	this.startActivity(intent);
	SettingsDialog mSettingsDialog = SettingsDialog.newInstance(SettingsDialog.NET_SETTING_PAGE);
...
//	this.startActivity(intent);
	...
	Intent intent = new Intent();
	intent.setClass(MainActivity.this, HudActivity.class);
	this.startActivity(intent);
    }
...
//	checkWifiEnable();
	...
	DebugHandler.logWithToast(this, this.getResources().getString(R.string.connect_success), 2000);
	String[] keys = {"device","version"};
	String[] values = {"android","4.1.2"};
	ConnectStateManager.getInstance(getApplication()).getIpcProxy().sendMessage2Server(keys, values);
    }
	...
	if(isStarted && current - lastToastTime > 2000) {
	    DebugHandler.logWithToast(this, this.getResources().getString(R.string.connect_fail), 1000);
