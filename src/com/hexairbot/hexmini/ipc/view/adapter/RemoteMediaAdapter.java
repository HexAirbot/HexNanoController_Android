...
public class RemoteMediaAdapter extends BaseAdapter implements
	...
	if (mData.size() > 0 || position >= mData.size()) {
	    ...
	    DisplayThumbTask displayTask = new DisplayThumbTask(
			mediaFile, holder.downloadind, holder.waitingInd,holder.thumbView);
...
//	}
	...
	public DisplayThumbTask(MediaFile file, View dInd, View wInd,ImageView view) {
	    mMediaFile = file;
...
//		    .logd(TAG, "end download file:" + mMediaFile.remotePath);
	    ...
	    bitmap = MediaUtil.getMediaThumbnail(mContext, mMediaFile, bitmapCache);
	    return bitmap;
	}
